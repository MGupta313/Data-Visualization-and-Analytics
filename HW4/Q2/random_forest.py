...
class RandomForest(object):
	...
	def _bootstrapping(self, XX, n):
		...
		samples = np.random.randint(0, n, size = n) # sampled dataset
		labels = np.array(XX)  # class labels for the sampled records
		### Implement your code here
		#############################################
			
		pass
		#############################################
		return (labels[:, :-1][samples], labels[:, -1][samples])
		...
		for record in X:
			...
			if len(counts) == 0:
				...
				for tree in self.decision_trees:
					vote_in_bag = tree.classify(record)
					votes.append(vote_in_bag)
				counts = np.bincount(votes)
